...
(struct node (comp-posn alpha beta whose-move) #:transparent #:mutable)
...
;With this we need to replace all "(node-children curnode)" calls with a "send give-all-positions minboard".
(define (alpha-beta-max curnode depthleft)
  ...
  (define (one-child-check remaining-children)
    ...
    (let* ([childposn (car remaining-children)]
           [child (node childposn (node-alpha curnode) (node-beta curnode) (node-whose-move curnode))]
  ...
  (let* ([list-of-child-posns (send minboard give-all-positions)]) 
    (if (or (= depthleft 0) (null? list-of-child-posns)) 
        (if (equal? (node-whose-move curnode) "white") (evaluate-posn (node-comp-posn curnode)) (- 0 (evaluate-posn (node-comp-posn curnode)))) 
...
(define (get-best-move board-comp-pos) (first-alpha-beta-max (node board-comp-pos (- 0 inf) inf (get-field turn board-comp-pos)) 3))

(get-field move (get-best-move random-complete-position-2))

;(define returned-move (get-best-move random-complete-position-1))
;(define mboard (new board% (complete-board-position initial-complete-position)))
;(display (get-field board-position (get-field complete-board-position mboard)))
;(send mboard give-all-positions)